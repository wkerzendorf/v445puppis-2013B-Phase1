...
%---- BOX 1 ------------------------------------------------------------
%
% You should use this template for period 90A applications ONLY.
%
...
% Valid values: NORMAL, GTO, TOO, CALIBRATION
...
\Abstract{V445 Puppis was a unique nova.  Based on its slow lightcurve
 and spectra showing carbon but no hydrogen, it was almost certainly
 the first known helium nova, i.e., a nova triggered by unstable
 helium burning.  Intriguingly, the inferred white-dwarf mass is
 close to the Chandrasekhar limit, making the system a plausible type
 Ia supernova progenitor.  Its nature, though, is unclear.  The
 simplest explanation involves some type of helium-star donor, but a
 ...
 escape notice.  Unfortunately, currently, there only is
 pre-explosion photometry (which implies a surprisingly high
 luminosity).  Post explosion, the binary has remained obscured by
 the nova shell, as shown dramatically by NACO images taken in
 2005--2007.  The shell expands, though, and we propose to revisit
 Nova Puppis to determine whether the binary has reappeared and
 spectroscopy has become possible.}
...
% 2. PERIOD
% Valid values: 90
% Exception: VLT-XMM proposals, which are only accepted in odd periods,
% can specify the next Period also. 
% This parameter is NOT checked at the pdfLaTeX compilation.
%
% 3. INSTRUMENT
% Valid values: AMBER CRIRES EFOSC2 FEROS FLAMES FLASH FORS2 HARPS HAWKI ISAAC LABOCA MIDI NACO OMEGACAM SABOCA SHFI SINFONI SOFI SOFOSC Special3.6 SpecialAPEX SpecialNTT SpecialVLTI UVES VIMOS VIRCAM VISIR WFI XSHOOTER
% 
...
\ObservingRun{A}{92}{NACO}{2h}{any}{n}{0.8}{THN}{s}{}
...
\SpecialRemarks{In box 13, we ask for the observations to be scheduled
 as two 1-hour blocks separated by at least a week, so that the
 results of the first block can inform the settings for the
 second (see box 8B).}
...
% http://www.eso.org/sci/observing/proposals/countryselect.html
%
...
%---- BOX 7 ------------------------------------------------------------
% 
% If this proposal is part of a thesis work, uncomment the line below.
% Valid values for the second parameter: s, m, n
% This macro is NOT checked at the pdfLaTeX compilation.

%\Thesis{A. Student. Data important for PhD thesis and student will 
%  lead the project}{m}
...
\ScientificRationale{
%
 {\em This is a resubmission of a previous proposal, with comments
   addressed.}
 ...
 Despite decades of study, however, the observational evidence
 remains confusing.  On the one hand, there is no sign of a
 non-degenerate companion in some supernovae (e.g., SN 2011fe, Nugent
 et al.\ 2011, Nature 480, 344; Bloom et al.\ 2012, ApJ 744, L17) and
 some supernova remnants (Schaefer \& Pagnotta, 2012, Nature, 481,
 164; Kerzendorf et al.\ 2012, ApJ, submitted) as well as a lack of
 suitable single-degenerate progenitor systems from both observations
 (Di Stefano 2010, ApJ 712, 728; Gilfanov \& Bogd\'an 2010, Nature
 463, 924) and population synthesis (Ruiter et al.\ 2009, ApJ 699,
 2026; Mennekens et al.\ 2010, A\&A 515, 89).  On the other hand, a
 fair fraction of all SN Ia show evidence for circumstellar medium
 that is most easily interpreted as winds from a companion or
 ...
 The above problem can be circumvented in two ways.  First, in
 sources in which accretion is relatively fast, hydrogen is burnt
 steadily.  Such sources are identified observationally as supersoft
 sources.  Alternatively, one can have a helium-rich donor (e.g., a
 helium white dwarf, as in the AM CVn systems mentioned above; a
 subdwarf; or a helium giant).  For all above sources, however, there
 is another hurdle: as the layer of helium thickens, it in turn may
 ignite, leading to a {\em helium nova}.  The effect of these on
 growth has usually been ignored.  In part, this likely was because,
 until recently, no helium novae had been identified.
 ...
 The pretty nebula also poses a problem, however, in that the
 equatorial waist obscures the binary.  As a result, the only clues
 to its nature we currently have, are that pre-explosion V445 Pup had
 $V\simeq14.5$ and $V-K\simeq3.0$, and that it was variable at the
 10\% level.  At the distance of $8.2\pm0.5\,$kpc inferred from the
 expansion of the nebula by Woudt et al., and correcting for
 interstellar reddening $E_{B-V}\simeq0.5$, this corresponds to a
 luminosity of $L\simeq2000\,L_\odot$ and a color $(V-K)_0\simeq1.6$
 (the object may be more luminous and bluer if there is substantial
 circumstellar reddening; Woudt et al., ibid).

 The inferred luminosity is very high for a cataclysmic variable, and
 the system's nature thus remains unclear: is the companion very
 luminous (but it is too blue to be a symbiotic), is the accretion
 disk very large, or did the system host a supersoft source?
 Furthermore, does the absence of evidence for hydrogen reflect the
 absence of hydrogen, or an undetectable amount of hydrogen (as might
 happen in a supersoft source)?
 ...
 will allow a first empirical evalulation of the plausibility of such
...
\ImmediateObjective{We propose to use 2 hours of NACO AO imaging to
 determine whether V445 Pup has become visible again, and thus
 whether, from follow-up spectroscopy, it will be possible to
 ascertain its nature: Is the binary hydrogen or helium rich? How
 large and luminous is the companion? How large is the accretion
 disk? Did the system host a supersoft source?

 Specifically, we propose to search for a point source in the nova
 remnant.  In the previous NACO observations by Woudt et al.\ (March
 2005 to March 2007), the binary was hidden from view by dust
 obscuration.  With an artificial star experiment, we find a limit
 $K>17$ (see figure).  Given that the source was detected in 2MASS
 (pre-explosion) at $K=11.5$, the implied obscuration in early 2007
 is $A_K>5.5$.  For constant expansion, the dust column density and
 hence the extinction scale as $\rho R\propto R^{-2}\propto t^{-2}$,
 and thus in early 2013 it should have been reduced by a factor
 $\sim\!3.5$, resulting in $A_K>1.6$.  One derives a similar limit
 from the fact that in early 2009, the source still was faint
 ($K=14.1$; Woudt et al.), with much of the emission likely still
 from the nebula (hence, $A_K>3$ in early 2009, implying $A_K>1.4$ in
 early 2013).  Given this, the counterpart could in principle be as
 bright as $K\simeq13$ in early 2013.  (Of course, it can also be
 almost arbitrarily faint; our purpose is to check whether it has
 become observable!)

 To obtain a homogenous dataset, we propose to use the same
 NAOS/CONICA setup used by Woudt et al. We propose two OBs with 0.6 h
 on target, so we can detect a counterpart down to $K\simeq18.1$,
 about a magnitude fainter than possible in the observations by Woudt
 et al.  At this level, we will know whether spectroscopy of the
 source is possible now, whether it will become possible in the next
 few years, or whether we should revisit the source with imaging in
 $\ga\!5$ years.

 {\em Note on scheduling:} we ask for the the OBs to be separated by
 at least a week, so that we can check the first image and, if the
 source has become bright, use the second OB for a classification
 spectrum.
%
}
%
...
\MakePicture{pup2007_artstar.pdf}{angle=0,width=0.8\hsize}
\MakeCaption{Fig.~1: NACO observations taken March 2007 by Woudt et
 al., with artificial stars added (using a scaled version of nearby
 stars).  The artificial star is barely visible at $K=17.21$, and we
 infer a conservative limit of $K>17$.  Early 2013, dust extinction
 should have reduced by a factor~3.5 and the binary could have become
 as bright as $K\simeq13$.}
...
\WhyNights{%
 We estimate the required time empirically, using the data taken
 previously with the same instrument by Woudt et al.\ (2009, ApJ 706,
 738).  As seen in the figure, there was no central object visible in
 the NACO observations in 2007.  Using an artificial star experiment,
 we determined that a source with magnitude $K_{\rm s}=17$ would be
 visible.  We wish to obtain a detection or limit at a slightly
 fainter magnitude, so that we know we can take follow-up
 classification spectroscopy, or to know how long to wait.

 Thus, we propose to use the same setup as that used by Woudt et al.\
 (which has the additional benefit that it will be easier to use the
 images for possible studies of the nebula's evolution): detector
 integration time of 45s (using DoubleRdRstRd we incur 0.7 sec
 overhead), 4 coadds and 12 dithers with an overhead of 27 s each.
 Adding 16.25 minutes of telescope acquisition time, the implied
 total time is $t_{\rm OB}= 975 + 12 \times (27 + 4 \times (45 +
 0.7))\,\textrm{s} = 0.95\,$h, which, not coincidentally, nicely fits
 in one 1-hr OB.

 Two OBs would make it possible for us to detect the source down to a
 limiting magnitude of $K=18.1$, so that, if the source is still not
 detected, we know it will be pointless to return to this source for
 $\sim\!5$ years.

 We request taking the two Observation blocks a few weeks apart, so
 that, if a source is detected and relatively bright in the first OB,
 we could decide to do immediate spectroscopy on the target using the
 second one.
%
}

\TelescopeJustification{V445 Puppis is currently enshrouded in dust,
and we expect extinctions $A_K>1.6$ even during our observations.
...
From the work of Woudt et al., it is clear NACO is an excellent
choice for these observations.  It is also the best choice, as other
instruments require brighter, closer-by guide stars.  In particular,
...
An additional benefit of using NACO is that, as it was used before,
it will be easier to explore the morphological evolution of the
remnant.  In that respect, this is timely, since NACO will be
decommissioned soon.
%
}
...
%\Calibrations{sta}{Adopt a special calibration}
...
\LastObservationRemark{Van Kerkwijk and Lepo obtained FLAMES data Fall
...
\RequestedDataRemark{Data of V445 Puppis were taken with the same
 setup in 2005--2007.  We are interested to see the time evolution of
 this object, in particular to determine whether the central sources
 is becoming visible, and thus we require new observations.}

%
%---- BOX 10b -- ESO GTO/Public Survey Programme Duplications---------
%
...
%
%---- BOX 11 ------ Applicant(s) publications ---------------------
%
...
resolution spectroscopic search with the FLAMES instrument,'' ApJ,
submitted 
}
...
% given in this list. INCLUDE ONLY TARGETS REQUESTED FOR P90 !
...
%\UnsuitableTimes{A}{15-jan-13}{18-jan-13}{Insert explanation of unsuitable time here.}
%\UnsuitableTimes{B}{15-jan-13}{18-jan-13}{Insert explanation of unsuitable time here.}
%\UnsuitableTimes{C}{20-jan-13}{23-jan-13}{Insert explanation of unsuitable time here.}


%
%---- BOX 13 contd.. -- Scheduling Requirements 
%
...
%\TimeCritical{A}{12-nov-12}{14-nov-12}{Insert reason for time-critical observations.}
%\TimeCritical{D}{1-nov-12}{2-nov-12}{Insert reason for time-critical observations.}
%\TimeCritical{D}{17-nov-12}{18-nov-12}{Insert reason for time-critical observations.}
%\TimeCritical{D}{23-nov-12}{24-nov-12}{Insert reason for time-critical observations.}
...
% If you plan to use the a NGS, please specify the NGS name in target list.
...
%-----------------------------------------------------------------------
%
%\INSconfig{}{VISIR}{PRE-IMG}{Provide list of filters HERE}
...
%\INSconfig{}{VISIR}{BURST}{Provide list of filters HERE}
%\INSconfig{}{VISIR}{SPEC N-band LR}{-}
%\INSconfig{}{VISIR}{SPEC N-band MR}{Provide central wavelength(s) in the ranges (7.4-9.4), (10.2-13.0) HERE}
%\INSconfig{}{VISIR}{SPEC Q-band MR}{Provide central wavelength(s) in the ranges (17.1-19.0), (19.9-20.3) HERE}
...
%!!!! HAVE TO THINK ABOUT INSTRUMENT SETUP!!!!
%\INSconfig{}{NACO}{IMG 54 mas/px IR-WFS}{provide list of filters HERE}
%\INSconfig{A}{NACO}{IMG 27 mas/px IR-WFS}{Ks}
...
%
%\INSconfig{}{NACO}{SDI+4 IR-WFS}{comments}
%\INSconfig{}{NACO}{SDI+4 VIS-WFS}{comments}
%
...
\INSconfig{A}{NACO}{SPEC VIS-WFS}{provide the list of spectroscopic modes HERE}
% % 
%
%-----------------------------------------------------------------------
...
%-----------------------------------------------------------------------
%\INSconfig{}{HAWKI}{PRE-IMG}{provide list of filters (Y,J,H,Ks,CH4,BrG,H2,NB1060,NB1190,NB2090) HERE}
%\INSconfig{}{HAWKI}{IMG}{provide list of filters (Y,J,H,Ks,CH4,BrG,H2,NB1060,NB1190,NB2090) HERE}
%\INSconfig{}{HAWKI}{BURST}{Provide list of filters  (Y,J,H,Ks,CH4,BrG,H2,NB1060,NB1190,NB2090) HERE}
%\INSconfig{}{HAWKI}{FASTJITT}{Provide list of filters  (Y,J,H,Ks,CH4,BrG,H2,NB1060,NB1190,NB2090) HERE}
...
%\INSconfig{}{HARPS}{spectro-Thosimult}{EGGS}
...
%
% The available baselines for Period 90 are shown below.
