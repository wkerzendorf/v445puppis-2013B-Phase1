...
%
 ...
 equatorial waist obscures the binary. In the 2007 image by Woudt et al. the central star was not visible, however, our new image from late 2012 revealed that the dust had "settled" for the star to be visible (for comparison see Figure~\ref{fig:v445img}). This only took half of our allocated time so we changed the OB and obtained a spectrum. With a S/N=6 the quality was too low for us to extract any useful information.
 
 From pre-explosion imaging we measure an unusual high luminosity  for a cataclysmic variable, and
...
\ImmediateObjective{We propose to use 2 hours of NACO AO spectropscopy to
...
For constant expansion, the dust column density and
 ...
 ??? WE use Slit_86mas with spectroscopic mode S54_3_SK - high resolution (R~1400) - 1.79-2.57 um ??
...
\WhyNights{%
 ...
 Our previous observations used the acquisition template `NACO_img_acq_MoveToSlit'  with 00:17:41.00 and 4 coadds with 634.75~s each filling an OB with exactly one hour (to obtain an ABBA pattern). Which leaves  4 integration times of 634.75 s to fill an hour. We propose for two such OBs, which results in a S/N of 20 using the ETC
